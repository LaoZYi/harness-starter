...
3. **初始化** — 生成文档、规则、任务追踪和 32 个工作流技能命令
...
初始化后，项目中会生成 32 个 Claude Code 斜杠命令，覆盖完整开发生命周期。
...
| 命令 | 用途 | 来源 |
|------|------|------|
| `/spec` | 规格驱动开发——四阶段门控流程（规格→计划→任务→实施），在写代码前明确需求和验收标准 | agent-skills |
| `/adr` | 架构决策记录——用 MADR 模板记录技术选型和设计决策的理由、备选方案和后果，支持创建/查看/更新 | architecture-decision-record |
...
| `/plan-check` | 对计划做 8+1 维度结构化校验（需求覆盖 / 原子性 / 依赖 / 文件作用域 / 可验证 / 上下文 / 缺口 / Nyquist / Agent 工程化条件触发）+ 3 轮修订循环 | gsd-absorb |
| `/agent-design-check` | Agent 设计体检——4 维度（F3 Context Ownership / F5 State Unification / F8 Control Flow / F10 Small Focused Agents）校验涉及多 agent 的计划 | 12-factor-agents |
...
当项目类型为 `meta` 时，除上述 32 个通用技能外，还会额外生成 4 个 meta 专属命令（统一 `meta-` 前缀），用于微服务全局管理。
...
| 情况 | 用什么 | 为什么 |
|------|--------|--------|
| "老板说了个模糊方向，我不知道具体做什么" | `/ideate` | 多角度发散，从 32 个想法里筛出 5-7 个靠谱的 |
...
典型组合：`/brainstorm` 做设计 → `/spec` 定义验收标准 → `/adr` 记录架构决策 → `/write-plan` 制定计划。

#### 第一步¾：记录架构决策

> **类比**：`/adr` 是在施工前签的选型决议书——为什么选混凝土不选钢结构。

| 情况 | 用什么 | 为什么 |
|------|--------|--------|
| "要选数据库/框架/API 风格等" | `/adr` | 用 MADR 模板记录决策理由、备选方案和后果 |
| "做了不可逆的技术选择" | `/adr` | 半年后你会感谢现在写下"为什么" |
| "纯实现细节，没有架构选择" | 跳过，直接制定计划 | |

典型组合：`/spec` 定验收标准 → `/adr` 记录关键技术决策 → `/write-plan` 制定计划。
...
框架的 32 个技能模板改编自 3 个上游项目 + 2 个吸收项目。当上游更新时，可以检查变更：
