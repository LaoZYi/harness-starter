...
3. **初始化** — 生成文档、规则、任务追踪和 31 个工作流技能命令
...
初始化后，项目中会生成 31 个 Claude Code 斜杠命令，覆盖完整开发生命周期。
...
| `/plan-check` | 对计划做 8 维度结构化校验（需求覆盖 / 原子性 / 依赖 / 文件作用域 / 可验证 / 上下文 / 缺口 / Nyquist 合规）+ 3 轮修订循环 | gsd-absorb |
...
当项目类型为 `meta` 时，除上述 31 个通用技能外，还会额外生成 4 个 meta 专属命令（统一 `meta-` 前缀），用于微服务全局管理。
...
| 情况 | 用什么 | 为什么 |
|------|--------|--------|
| "老板说了个模糊方向，我不知道具体做什么" | `/ideate` | 多角度发散，从 31 个想法里筛出 5-7 个靠谱的 |
...
框架的 31 个技能模板改编自 3 个上游项目 + 2 个吸收项目。当上游更新时，可以检查变更：
