...
3. **初始化** — 生成文档、规则、任务追踪和 30 个工作流技能命令
...
初始化后，项目中会生成 30 个 Claude Code 斜杠命令，覆盖完整开发生命周期。
...
当项目类型为 `meta` 时，除上述 30 个通用技能外，还会额外生成 4 个 meta 专属命令（统一 `meta-` 前缀），用于微服务全局管理。
...
框架的 30 个技能模板改编自 3 个上游项目 + 2 个吸收项目。当上游更新时，可以检查变更：
