...
4. **工作流技能**：默认生成 28 个结构化开发技能命令（融合 superpowers + compound-engineering + gstack），覆盖构思、设计、计划、执行、评审、安全、沉淀、自我进化全生命周期。可通过 `--no-superpowers` 关闭。
5. **首次分析**：初始化后 current-task.md 预填分析任务，AI 打开项目自动补全文档。
6. **升级**：对已接入的项目做增量升级，支持 diff 预览、选择性升级和自动备份。
