...
3. **初始化**：根据项目类型和探测结果生成 23 个文档/配置文件，包括 AGENTS.md、CLAUDE.md、.claude/rules/、docs/、.agent-harness/、.github/ 模板。
4. **升级**：对已接入的项目做增量升级，支持 diff 预览、选择性升级和自动备份。
5. **运维**：doctor（健康检查）、export（画像导出）、stats（任务统计）。
6. **扩展**：插件机制，用户可在 .harness-plugins/ 下放自定义规则和模板。
...
| 命令 | 作用 |
|------|------|
| `harness init <target>` | 交互式初始化（5 个问题 + 自动探测） |
