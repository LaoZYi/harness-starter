...
- `src/agent_harness/templates/superpowers/`：结构化工作流技能模板（26 个命令 + 1 个规则），默认启用，可通过 `--no-superpowers` 关闭。融合了 obra/superpowers（14 个基础技能）、EveryInc/compound-engineering-plugin（6 个增强技能）、garrytan/gstack（5 个运维技能）和 1 个本地原创技能（lint-lessons，灵感来自 Karpathy 的 LLM Wiki 模式）。
