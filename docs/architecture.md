...
具体而言：
- **43 个工作流技能** = Agent 的工具集（覆盖构思→设计→实施→评审→沉淀全生命周期）
...
- `src/agent_harness/templates/superpowers/`：结构化工作流技能模板（39 个命令 + 1 个规则），默认启用，可通过 `--no-superpowers` 关闭。融合了 obra/superpowers（14 个基础技能）、EveryInc/compound-engineering-plugin（6 个增强技能）、garrytan/gstack（5 个运维技能）、addyosmani/agent-skills（1 个吸收技能 + 反合理化增强）、joelparkerhenderson/architecture-decision-record（1 个吸收技能）、spencermarx/open-code-review（评审辩论方法论增强）、3 个本地原创技能（lint-lessons、evolve、squad）、5 个文档场景技能（lfg-doc、outline-doc、draft-doc、review-doc、finalize-doc，2026-04-27 D 方案）和 1 个幻灯片场景入口（lfg-slide，2026-04-27，Issue #56 触发条件 1，与 lfg-doc 严格平行复用底层 doc skill）。
...
- `src/agent_harness/templates/<type>/`：各项目类型的专属规则模板（backend-service、web-app、cli-tool、worker、mobile-app、monorepo、data-pipeline、library、document、slide-deck 各有 1 个专属规则文件）。
- `src/agent_harness/presets/`：10 种项目类型的 JSON 预设（document / slide-deck 含），含 `workflow_skills_summary` 指定项目类型重点技能。
...
- 43 个工作流技能（`/lfg`、`/tdd`、`/spec` 等）都是**用户主动触发**的——开发者明确知道自己想做什么（"开始全流程"、"测试驱动开发"），不需要模型自行猜测
