...
2. **计划阶段** — 运行 `/write-plan` 编写实现计划（2-5 分钟粒度的任务）
3. **执行阶段** — 运行 `/execute-plan` 按计划逐步实施，或 `/tdd` 进行测试驱动开发
4. **验证阶段** — 运行 `/verify` 在告知用户"完成"之前做全面验证
5. **评审阶段** — 运行 `/multi-review` 多人格评审，或 `/request-review` 准备评审请求
6. **沉淀阶段** — 运行 `/compound` 提炼经验写入知识库

> 想全自动完成？运行 `/lfg` 一键串联上述全部阶段。

## 项目重点技能

CLI 工具重点技能：`/tdd`（命令行为测试）、`/write-plan`（子命令规划）、`/verify`（跨平台验证）、`/git-commit`（结构化提交）
...
- 需要探索方向和创意 → `/ideate`
- 不确定从哪里开始 → `/use-superpowers` 或 `/brainstorm`
- 任务复杂需要规划 → `/write-plan` → `/execute-plan`
- 想全自动完成 → `/lfg`
- 需要写新功能 → `/tdd`
- 遇到 bug → `/debug`
- 多个独立子任务 → `/dispatch-agents` 或 `/subagent-dev`
