...
3. **计划阶段** — 运行 `/write-plan` 编写实现计划（2-5 分钟粒度的任务）
4. **执行阶段** — 运行 `/execute-plan` 按计划逐步实施，或 `/tdd` 进行测试驱动开发
5. **验证阶段** — 运行 `/verify` 在告知用户"完成"之前做全面验证
6. **评审阶段** — 运行 `/multi-review` 多人格评审，或 `/request-review` 准备评审请求
7. **沉淀阶段** — 运行 `/compound` 提炼经验写入知识库
