...
#### 下游消费者门禁（Anti-Laziness Gate #4）

每步开始前，校验该步骤的前置产物存在且非空：

1. 前置步骤的产出文件是否存在？→ 不存在则**停下**，回退到前置步骤
2. 文件是否非空（> 0 字节）？→ 空文件说明前置步骤未真正完成
3. 文件内容是否非占位符（不全是 TODO/TBD/待补充）？→ 是占位符则**警告**

> 参见 `.claude/rules/anti-laziness.md` 门禁 4 的完整检查方式。

对每个任务：

1. **校验前置产物**（下游消费者门禁）
2. 在 `.agent-harness/current-task.md` 中标记当前任务为 `in_progress`
3. 严格按照计划描述实现，不擅自扩展范围
4. 每完成一个原子操作就保存进度
5. 使用 `make test` 运行验证
6. 使用 `make check` 检查代码质量
