...
- **作为 `/execute-plan` 启动前的最后一道门**（来自腾讯 JK Launcher 的「闸门总控」实践）：即将动手写代码前再跑一次**轻量版** plan-check，重点只审「可行性 + 风险 + 前置产物就位度」三项（维度 6/7 + 门禁 4），不重复审计维度 1-3。这一步的目的是防"计划写完到实施之间隔了多轮对话、上下文已漂移"导致带风险开工
...
| 场景 | 范围 | 典型耗时 |
|---|---|---|
| `/write-plan` 收尾 | 完整 8+1 维度 | 长 |
| execute 前门 | 维度 6（上下文适配）+ 维度 7（缺口）+ 门禁 4（前置产物非空）| 短 |
| 独立审查 | 按需勾选 | 视情况 |
