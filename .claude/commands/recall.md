# 按需检索历史知识

在 `.agent-harness/lessons.md` 和 `.agent-harness/task-log.md` 中按关键词检索，取回与当前任务相关的历史教训或完成记录。

当前项目：`Agent Harness Framework`（cli-tool）

## 背景

为避免上下文窗口膨胀，`task-lifecycle` 规则默认只让 AI 读 `.agent-harness/memory-index.md`（精华索引）。当 index 提示某话题可能相关（或你凭经验判断需要深入），用本技能做定向检索，而不是读取全量历史文件。

## 适用场景

- 开始任务时 memory-index 命中某关键词，需要读对应 lessons 条目的完整内容
- 实施过程中怀疑以前踩过类似坑，想验证是否有历史教训
- 评审或验证时需要引用历史决策作为依据
- 归档任务前想确认没遗漏相关的过往经验

## 参数

- `<关键词>`：必填（`--map` 模式除外）。在目标文件中做不区分大小写的子串匹配
- `--history`：可选。只搜索 task-log.md（跳过 lessons.md 和 references/）
- `--lessons`：可选。只搜索 lessons.md（跳过 task-log.md 和 references/）
- `--refs`：可选。只搜索 `.agent-harness/references/*.md`（checklist + 用户添加的自定义参考文档）
- `--map`：可选。**全项目目录导航模式**——不接关键词，扫描所有 `ABSTRACT.md` 给出整个项目的目录语义地图，供 AI 定位"该去哪个目录找"
- `--all`：可选。返回所有匹配节的完整内容；不加时默认只返回匹配到的二级标题列表

**默认搜索范围**：lessons.md + task-log.md + references/*.md（即"三合一"）。加 `--lessons` / `--history` / `--refs` 任一后，仅搜索该范围。`--map` 独立模式，与关键词互斥。

## 执行步骤

### 第 0 步：`--map` 模式（独立分支）

若传 `--map`，走目录导航模式，忽略 lessons/task-log/references：

1. 运行 `find . -name ABSTRACT.md -not -path '*/node_modules/*' -not -path '*/.git/*' | sort`
2. 对每个 ABSTRACT 文件读取并输出：`<相对路径>：<ABSTRACT 内容一句话>`
3. 若用户后续要深入某目录，建议读同目录的 `OVERVIEW.md` 做 L1 导航
4. 若项目根全无 ABSTRACT.md：提示「本项目尚未维护目录地图，按 `/recall <关键词>` 走传统三合一检索」
5. 若同时传了关键词：报错「`--map` 不接关键词，两种模式二选一」

`--map` 模式到此结束，不进入后续步骤。

### 第 1 步：定位候选文件

根据参数决定要搜的文件：

| 参数 | 搜索范围 |
|------|---------|
| （默认） | `.agent-harness/lessons.md` + `.agent-harness/task-log.md` + `.agent-harness/references/*.md` |
| `--lessons` | 仅 `.agent-harness/lessons.md` |
| `--history` | 仅 `.agent-harness/task-log.md` |
| `--refs` | 仅 `.agent-harness/references/*.md`（a11y / perf / security / testing 等 checklist） |

文件不存在时告知用户并继续搜其它文件。

### 第 2 步：Grep 匹配

对每个候选文件使用 Grep 工具（`output_mode: "content"`, `-i: true`, `-n: true`）搜索关键词。

### 第 3 步：定位匹配所在节

每个匹配行所属的二级标题（`## YYYY-MM-DD ...`）即为"匹配节"。合并同一节的多条命中。

### 第 4 步：返回结果

**默认模式**（未传 `--all`）：列出所有匹配节的标题和一句话摘要，格式：

```
来自 lessons.md:
  - 2026-04-09 **命令重命名后模板也要全量扫描** — 避免文档/模板不同步
  - 2026-04-08 dogfood 命令展平 — project.json 嵌套 commands 字典需展平

来自 task-log.md:
  - 2026-04-10 深度完善项目类型功能差异化
```

**全文模式**（`--all`）：在上述列表基础上，追加每个匹配节的完整 Markdown 内容。

### 第 5 步：Grep 未命中时启动 BM25 兜底（二级检索）

若 Grep 在第 2-4 步完全无匹配（返回 0 节），**自动串 BM25 兜底检索**：

```bash
.agent-harness/bin/memory search "<关键词>" --scope all --top 5
```

灵感自 [mksglu/context-mode](https://github.com/mksglu/context-mode) 的 FTS5/BM25 理念，本项目零依赖纯 stdlib 实现（见 `src/agent_harness/memory.py`）。

**为什么要 BM25 兜底**：
- Grep 是精确子串匹配——输错一个字或用了同义词就会漏
- BM25 是相关性排序——按 token 重要度（IDF）加权、按文档长度归一化，把"最像查询的段落"排到前面，即使关键词不完全一致
- 分词支持中英混合（英文按 `\w+` 切、中文按字符 1-gram）

**判断标准**：Grep 命中 ≥ 1 节 → 不启动兜底（节省 token）；Grep 返回空 → **必须**跑一次 BM25；把两级结果合并呈现给用户时，标注哪些来自 Grep、哪些来自 BM25。

### 第 6 步：建议后续动作

若 Grep + BM25 都返回空：告知用户"无历史记录涉及此关键词"。

若返回结果很多（> 5 节）：建议用户细化关键词或加上 `--lessons`/`--history` 缩小范围。

## 三段式检索（推荐）

吸收自 [volcengine/OpenViking](https://github.com/volcengine/OpenViking) 的"文件系统即上下文"思想——用 ABSTRACT/OVERVIEW 把目录语义承载在数据层，AI 不再靠读全文猜目录用途。

当你**不确定话题落在哪个目录**时，推荐按三段式检索：

1. **L0 扫 ABSTRACT**：先跑 `/recall --map`，看哪个目录可能相关
2. **L1 读 OVERVIEW**：对命中目录读其 `OVERVIEW.md`，按"关键文件列表"和"推荐阅读顺序"挑具体文件
3. **L2 grep 原文**：在目标文件上跑 `/recall <关键词>` 或直接 Grep

相比"默认三合一 grep"，三段式减少无效匹配和上下文浪费。适用范围见 `.claude/rules/documentation-sync.md` 的目录导航层白名单。

## 使用示例

```
/recall --map                # 扫全项目 ABSTRACT.md 给目录地图
/recall 升级                 # 搜 lessons + task-log + references（默认三合一）
/recall 合并 --all           # 完整展开所有"合并"相关条目
/recall memory --history     # 只搜 task-log
/recall 三方合并 --lessons   # 只搜 lessons
/recall CORS --refs          # 只搜 references（找 security-checklist 的 CORS 节）
/recall LCP --refs           # 只搜 references（找 performance-checklist 的 LCP 节）
```

## 反合理化

| 你想说的 | 为什么不行 |
|---------|-----------|
| "我直接读整个 lessons.md 更快" | 正是本技能要解决的上下文膨胀问题；用本技能定向检索 |
| "memory-index 没命中就肯定没有" | index 可能未及时更新；`/recall` 是对 index 的兜底 |
| "Grep 一下就行不用走技能" | Grep 得到的是匹配行不是匹配节；本技能负责节定位和摘要 |
| "搜不到就说明一定没犯过" | 历史 < 现在；你可能遇到新场景。用技能确认后仍无记录 → 考虑做完用 /compound 首次记录 |
| "随便取前 3 个就行" | 可能漏掉关键节；默认列全部标题让用户/AI 判断相关性 |
| "中文关键词 Grep 不好用" | Grep 底层是 ripgrep，UTF-8 原生支持；直接用中文关键词即可 |
| "Grep 没命中就是没有，不用再跑 BM25" | Grep 要完全一致；你可能用了同义词或错字。BM25 是必跑兜底，不是可选项 |
| "我直接 grep 就行，不需要 `--map`" | 三段式是"不确定话题在哪里"时用的，已知目录时直接 grep 本身没错。但对新进项目 AI，`--map` 比挨个读 README 快 |
