...
1. **先判断是不是运维/元任务**——如果是，告知用户走 CLI 不走 /lfg：
   ...
   | 重建分层记忆索引 | `harness memory rebuild <target>` | 工具类 |
...
GitHub 和 GitLab 的 Issue 编号不同。必须先按标签 + 标题查找 GitLab 对应 Issue，再关闭：

```bash
# 第 1 步：按 evolution 标签列出 open Issue，用标题匹配找到对应编号
GL_ISSUE=$(curl -sS --header "PRIVATE-TOKEN: $GITLAB_TOKEN" \
  "http://192.168.4.102/api/v4/projects/ai-x%2Fzjaf-harness/issues?labels=evolution&state=opened&per_page=100" \
  | python3 -c "
import sys, json
title_keyword = '<GitHub Issue 标题中的关键词>'
issues = json.load(sys.stdin)
for i in issues:
    if title_keyword.lower() in i['title'].lower():
        print(i['iid'])
        break
" 2>/dev/null)
...
if [ -n "$GL_ISSUE" ]; then
  curl -sS --request POST \
    ...
    --data '{"body":"✅ 已通过 /lfg 集成完成。提交：<首个SHA>..<末个SHA>"}' \
    "http://192.168.4.102/api/v4/projects/ai-x%2Fzjaf-harness/issues/$GL_ISSUE/notes"
  curl -sS --request PUT \
    --header "PRIVATE-TOKEN: $GITLAB_TOKEN" \
    --header "Content-Type: application/json" \
    --data '{"state_event":"close"}' \
    "http://192.168.4.102/api/v4/projects/ai-x%2Fzjaf-harness/issues/$GL_ISSUE"
fi
```

> **禁止使用 GitLab API 的 `search=` 参数查找 Issue**——该参数对中文标题和混合语言内容匹配不可靠。必须用 `labels` 过滤后在本地做标题匹配。
...
在 `.agent-harness/current-task.md` 中实时记录完整上下文。使用以下结构化格式确保中断恢复时不丢失关键信息：
...
### Context（上下文）
复杂度：中 | 通道：标准 | 基线 commit：abc1234 | 工作分支：feat/user-search

### Assumptions（假设清单）
- 假设：使用 REST API | 依据：现有接口均为 REST
- 假设：搜索不需要全文索引 | 依据：数据量 <10k
...
### Decisions（关键决策）
- 使用前端过滤而非 API 搜索（数据量小，减少后端改动）
- 历史参考：lessons.md "API 端点需注意分页参数验证"

### Files（涉及文件）
- `src/views/UserList.vue` — 新增搜索组件
- `src/api/users.ts` — 无需修改
- `tests/UserList.spec.ts` — 新增搜索测试
...
- [x] 规格定义 — docs/superpowers/specs/2026-04-07-user-search-spec.md（需求明确，已跳过/已完成）
- [x] 计划 — docs/superpowers/specs/2026-04-07-user-search-plan.md（12 步）
- [x] 实施 — 12/12 步完成，12 个 commit
- [x] 自检 — 验收标准 4/4 通过，规则合规 5/5
- [x] 评审 — 第 1 轮 PASS WITH CONDITIONS → 修复 1 轮 → 第 2 轮 PASS
- [x] 验证 — pytest ✓ (48/48), ruff ✓, 验收 4/4, 穷举验证 ✓（涉及文件读写）
- [x] 质量对比 — 测试 +6, Lint -1
- [x] 沉淀 — 经验提炼完成，ADR 状态已更新（或无 ADR），快速 lint 通过
- [x] 完成报告 — 已生成
- [x] 待验证 — 用户确认通过
- [ ] 归档与收尾 — 待用户选择合并方式
```
...
| 阶段 | 自动/确认 | 何时停下 |
|------|----------|---------|
| 理解任务 | ⚙️ 自动 → 🔴 确认 | current-task 有未完成任务；任务描述模糊 |
| 加载历史 | ⚙️ 自动 | 不停 |
| 复杂度评估 | 🔴 确认 | 展示判断 + 假设清单，用户可覆盖 |
| 环境准备 | ⚙️ 自动 | 基线测试失败 |
| 构思 | 🔴 确认 | 用户选择方向 |
| 规格定义 | ⚙️ 自动 → 🔴 确认 | 触发条件满足时展示规格等确认 |
| 计划 | 🔴 确认 | 展示计划摘要等确认 |
| 实施 | ⚙️ 自动 | 连续 3 步失败或遇阻 |
| 自检 | ⚙️ 自动 | 验收标准或规则合规未满足时补做 |
| 评审 | ⚙️ 自动 | 不停 |
| 修复 | ⚙️ 自动 | 3 轮未收敛（绝对上限 6 轮） |
| 验证 | ⚙️ 自动 | 穷举验证失败；安全审计 Critical/High |
| 质量对比 | ⚙️ 自动 | 不停 |
| 沉淀 + 知识维护 | ⚙️ 自动 | 不停 |
| 待验证 | 🔴 确认 | 展示完成报告，等用户确认 |
| 收尾 | 🔴 确认 | 选择合并方式 |
...
| 2.5 规格 | `/spec` |
| 3 计划 | `/write-plan`、`/adr`、`/source-verify`（API 类任务必跑）、`/todo`（拆分粒度） |
