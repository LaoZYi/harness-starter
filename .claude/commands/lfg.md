...
**在制定任何计划之前**，先读取项目的历史经验和约束：

1. 读 `.agent-harness/lessons.md` — 搜索与本次任务相关的已知问题和解决方案
2. 读 `.agent-harness/task-log.md` 最近 5 条记录 — 了解项目最近做了什么
3. 读 `docs/architecture.md` — 了解模块边界，避免在错误的位置写代码
4. 读 `docs/product.md` — 了解现有功能，避免与已有功能冲突
5. 读 `AGENTS.md` — 了解硬规则约束，不可违反
6. 读 `docs/decisions/` — 浏览已有架构决策记录（ADR），了解历史技术选型和约束
...
如果用户说"回退到第 N 步"：
1. 在回滚前先打一个快照 tag `lfg/pre-rollback`（保护当前状态，让回滚本身也可撤销）
2. `git reset --hard lfg/step-N`（在工作分支上）
3. 更新 `current-task.md` 进度，将第 N+1 步之后的 checkbox 改回 `[ ]`
4. 从第 N+1 步继续

### 全量回滚（回退到任务开始前）

1. 在回滚前先打一个快照 tag `lfg/pre-rollback`
2. 读取 `current-task.md` 中记录的基线 commit
3. `git reset --hard <基线commit>`（在工作分支上）或删除 worktree
4. 清理 `current-task.md` 中的 LFG 进度和所有 `lfg/*` tag
5. 告知用户已回退到任务开始前的状态
