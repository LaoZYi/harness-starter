...
curl -sS --request POST \
  ...
  "http://192.168.4.102/api/v4/projects/ai-x%2Fzjaf-harness/issues/<编号>/notes"
# 再关闭
curl -sS --request PUT \
  --header "PRIVATE-TOKEN: $GITLAB_TOKEN" \
  --header "Content-Type: application/json" \
  --data '{"state_event":"close"}' \
  "http://192.168.4.102/api/v4/projects/ai-x%2Fzjaf-harness/issues/<编号>"
```

> 注意：GitHub 和 GitLab 的 Issue 编号可能不同。如果只知道一端的编号，用标题搜索另一端的对应 Issue 再关闭。
