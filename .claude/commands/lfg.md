...
3. **明确验收标准**：这个任务做到什么程度算"完成"？列出 3-5 条可验证的标准。模糊需求(Add X / Fix Y / Optimize Z 等)按 `.agent-harness/references/requirement-mapping-checklist.md` 末尾「模糊需求 → 可验证目标转换模板」的 7 类 pattern 转换为机械化命令(测试/grep/diff/curl)+ 数字阈值,人工解释不算可验证(Issue #51 吸收 Karpathy Goal-Driven Execution)
...
| 想"顺手改进"邻居代码(本任务无关的格式 / 注释 / 重构 / dead code 清理) | **禁止**(违反 simplicity.md 准则 2 Surgical Changes)。**提一句**让用户决定,不主动改;真要改走独立 `unplanned:` commit + 显式说明 |
...
4. **公共规则合规检查**：逐条核对项目规则，确认没有违反
   ...
   | **simplicity**(Issue #51) | diff 中每行改动都能追溯到用户请求? 有没有顺手改邻居代码 / 加无用抽象 / 写 200 行能搞定 50 行的过度膨胀? 单次使用代码加抽象了吗? |
...
| 元任务（非单任务流，不由 /lfg 驱动） | `/lfg`（self-reference would be infinite recursion）、`/which-skill`（meta skill-selector, peer to /lfg）、`/write-skill`（manually invoked when authoring new skills）、`/evolve`（periodic self-evolution, reverse-triggers /lfg via evolution Issues）、`/health`（periodic code-quality snapshot, not part of single-task flow）、`/retro`（periodic engineering retrospective）、`/process-notes`（product-notes processing, different domain）、`/digest-meeting`（meeting-transcript processing, upstream of /lfg (produces current-task.md as input, not a pipeline stage)）、`/pressure-test`（periodic skill TDD (monthly + on anti-rationalization table additions), orthogonal to /lfg single-task pipeline）、`/lfg-doc`（doc-scenario entrypoint, parallel to /lfg (code scenario); not part of /lfg pipeline）、`/lfg-slide`（slide-scenario entrypoint, parallel to /lfg-doc (doc scenario); not part of /lfg pipeline）、`/outline-doc`（doc-scenario skill, called from /lfg-doc not /lfg）、`/draft-doc`（doc-scenario skill, called from /lfg-doc not /lfg）、`/review-doc`（doc-scenario skill, called from /lfg-doc not /lfg）、`/finalize-doc`（doc-scenario skill, called from /lfg-doc not /lfg; replaces /verify+/git-commit+/finish-branch for doc）、`/team-spec`（team-* 是局部场景化编队（吸收 CCGS team-* 模式，Issue #55），用户显式调用，不是 /lfg 阶段技能；功能上覆盖 /lfg 阶段 2.5-3 但作为独立入口存在）、`/team-implement`（team-* 是局部场景化编队（吸收 CCGS team-* 模式，Issue #55），用户显式调用，不是 /lfg 阶段技能；功能上覆盖 /lfg 阶段 3-4 但作为独立入口存在）、`/team-review`（team-* 是局部场景化编队（吸收 CCGS team-* 模式，Issue #55），用户显式调用，不是 /lfg 阶段技能；功能上覆盖 /lfg 阶段 5-6 但作为独立入口存在）、`/team-doc`（team-* 是局部场景化编队（吸收 CCGS team-* 模式，Issue #55），跳过 /lfg-doc 前置探索阶段；用户显式调用，不进 /lfg 单任务流） |
