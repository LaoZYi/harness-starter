# 无障碍检查清单（Accessibility）

> L2 温知识层 — 与 `web-app` 类型规则配合使用。按需通过 `/recall --refs 关键词` 加载。
>
> 条目保留英文术语（WCAG/ARIA/Tab/Focus 等行业共识），序言已汉化。

WCAG 2.1 AA 合规的快速参考。适用于 web-app 和 mobile-app 类型项目。
